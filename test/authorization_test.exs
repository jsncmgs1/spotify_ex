defmodule SpotifyAuthorizationTest do
  ...
  describe ".url" do
    test "with scopes" do
      url = "https://accounts.spotify.com/authorize?client_id=123&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A4000&scope=foo-bar%20baz-qux"
      assert(Spotify.Authorization.url == url)
    end

    test "without scopes" do
      Application.put_env(:spotify_ex, :scopes, [])
      url = "https://accounts.spotify.com/authorize?client_id=123&response_type=code&redirect_uri=http%3A%2F%2Flocalhost%3A4000"
